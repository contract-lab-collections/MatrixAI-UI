...
function formatOne(column) {
	...
	switch (t.showType) {
		...
		case "btn":
			t.render = (text, record, index) => {
				return (<span className="mini-btn" onClick={()=>t.fun(text, record, index)}>{t.btnLabel||text}</span>);
			};
			break;
		case "btn-link":
			t.render = (text, record, index) => {
				return (<span className="btn-link" onClick={()=>t.fun(text, record, index)}>{t.btnLabel||text}</span>);
			};
			break;
		case "date":
		...
		case "link":
			...
			t.render = (text, record, index) => {
				...
				return <NavLink className="link" to={tpl}>{text}</NavLink>;
			};
			break;
		case "currency":
			t.render = (text, record, index) => {
